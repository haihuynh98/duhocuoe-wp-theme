...
function render_form_contact($post): void
{
    ...
    $address = get_post_meta($post->ID, '_ct_address', true);
    $phone = get_post_meta($post->ID, '_ct_phone', true);
    $email = get_post_meta($post->ID, '_ct_email', true);
    $fanpages = get_post_meta($post->ID, '_ct_fanpages', true);
    $gmap = get_post_meta($post->ID, '_ct_iframe_gmap', true);

    echo('<table class="form-table" role="presentation">');
    echo('<tbody>');

    echo('<tr>');
    echo('<th scope="row"><label for="address">Address</label></th>');
    echo('<td>');
    echo('<input type="text" id="address" name="address" value="' . esc_attr($address) . '" style="width:100%" />');
    echo('</td></tr>');

    echo('<tr>');
    echo('<th scope="row"><label for="contact_phone">Contact phone</label></th>');
    echo('<td>');
    echo('<input type="text" id="contact_phone" name="contact_phone" value="' . esc_attr($phone) . '" style="width:100%" />');
    echo('</td></tr>');

    echo('<tr>');
    echo('<th scope="row"><label for="email">Email</label></th>');
    echo('<td>');
    echo('<input type="text" id="email" name="email" value="' . esc_attr($email) . '" style="width:100%" />');
    echo('</td></tr>');

    echo('<tr>');
    echo('<th scope="row"><label for="fanpages">Fanpages</label></th>');
    echo('<td>');
    echo('<input type="text" id="fanpages" name="fanpages" value="' . esc_attr($fanpages) . '" style="width:100%" />');
    echo('</td></tr>');
    ...
    echo('<tr>');
    echo('<th scope="row"><label for="iframe_gmap">Iframe of google map</label></th>');
    echo('<td>');
    echo('<textarea id="iframe_gmap" name="iframe_gmap" rows="4" style="width:100%" />');
    echo(esc_attr($gmap));
    echo ('</textarea>');
    echo('</td></tr>');
...
function contact_meta_box_save($post_id)
{
    if (isset($_POST['address'])) {
        update_post_meta($post_id, '_ct_address', $_POST['address']);
    }
    ...
    if (isset($_POST['fanpages'])) {
        update_post_meta($post_id, '_ct_fanpages', $_POST['fanpages']);
    }
 if (isset($_POST['short_code_cf7'])) {
        update_post_meta($post_id, '_ct_short_code_cf7', $_POST['short_code_cf7']);
    }
 if (isset($_POST['iframe_gmap'])) {
        update_post_meta($post_id, '_ct_iframe_gmap', $_POST['iframe_gmap']);
    }
